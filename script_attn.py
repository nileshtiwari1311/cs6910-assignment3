...
#--------------------------------------------------------
# b_sz - batch size
# h_sz - hidden layer size
# n_lyr - number of layers
# seq_len - sequence length
# emb_sz - embedding size
# tgt_vcb_sz - target vocab size
...
PAD_token = "^" # padding
UNK_token = "$" # unkown token (if the test data has any character that is not in the vocabulary, it will be replaced by UNK token)
...
class Script:
    def __init__(self, name):
        self.name = name
        # adding SOS, EOS, PAD and UNK tokens into map
    ...
    def addWord(self, word):
        for char in word:
            self.addChar(char)
    # function that adds a character to the vocabulary
...
# appends sos or eos accordingly
...
class Encoder(nn.Module):
    def __init__(self, cell_type, input_size, embedding_size, hidden_size, num_layers, dp, bidir=False):
        ...
        if self.num_layers == 1:
            dp = 0.0
        # appropriate cell according to cell type
    ...
    def forward(self, x):
        ...
        # embedding shape: (seq_len, b_sz, emb_sz)
        # handle cases of LSTM and Bidirectional accordingly
        cell = None
        if self.cell_type == "LSTM":
            outputs, (hidden, cell) = self.cell(embedding)
            # cell shape: (2*n_layers, b_sz, h_sz) in case of bidirectional=True
            # convert it to (1, b_sz, h_sz)
            # take mean of last layer's forward and backward matrices
            if self.bidir:
                b_sz = cell.size(1)
        ...
        if self.bidir:
            ...
            # outputs is of shape: (seq_len, b_sz, 2*h_sz)
            # so convert it to (seq_len, b_sz, h_sz)
...
class AttnDecoder(nn.Module):
    def __init__(
        ...
        # last fully connected layer that will produce output of size (1, b_sz, tgt_vcb_sz) from the output of the decoder cell
        # concatenated with the context vector
        self.fc = nn.Linear(self.hidden_size * 2, self.output_size)

    def forward(self, x, enc_out, hidden, cell):
        # x shape: b_sz we want it to be (1, b_sz), one character at a time
        x = x.unsqueeze(0)

        embedding = self.dropout(self.embedding(x))
        # embedding shape: (1, b_sz, emb_sz)
        ...
        attn_w = F.softmax(score, dim=2) # (1, b_sz, seq_len) # compute attention weights using score
        context = attn_w.permute(1, 0, 2) @ enc_out.permute(1, 0, 2) # (b_sz, 1, h_sz)
        context = context.permute(1, 0, 2) # (1, b_sz, h_sz) # compute context vector
        #----------------------------------------
        ...
        # predictions shape: (1, b_sz, tgt_vcb_sz), to send it to
        # loss function we want it to be (b_sz, tgt_vcb_sz)
...
class Seq2Seq(nn.Module):
    ...
    # forward prop function that generates output of size (tgt_len, b_sz, tgt_vcb_sz) and
    # attention matrix of size (tgt_len, b_sz, src_len)
    def forward(self, source, target, teacher_force_ratio=0.5):
        ...
        # hidden and cell from encoder of size: (1, b_sz, h_sz)
        # repeat it for number of decoder layers as the same output from the
        ...
        # tensor to store attention of each step in sequence
        attn_matrix = torch.zeros(target_len, batch_sz, source_len).to(device) # (tgt_len, b_sz, src_len)

        # first input to the decoder, <SOS> token
        x = target[0]

        for t in range(1, target_len):
            # Use previous hidden, cell as context from encoder at start
            # from the next iteration, use self hidden, cell
            ...
            # Take the argmax over the tgt_vcb_sz
...
def sum_accuracy(preds, target):
    # neglect padding indices in target sequences
...
def evaluateModel(model, dataloader, criterion, b_sz=32):
    ...
    with torch.no_grad():
        for (input_seq, target_seq) in dataloader:
            ...
            # no teacher forcing in inference mode
            ...
            # output is of shape: (tgt_len, b_sz, tgt_vcb_sz)
            # convert it to (b_sz*tgt_len, tgt_vcb_sz) for calculating cross-entropy loss
            # similarly for target_seq to (b_sz*tgt_len)
...
def trainModel(model, criterion, optimizer, train_dataloader, valid_dataloader, num_epochs, batch_size=32):
    ...
    # stores the best model obtained
